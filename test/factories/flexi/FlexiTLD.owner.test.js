...
describe("FlexiPunkTLD (onlyOwner)", function () {
  ...
  beforeEach(async function () {
    ...
    sfsContract = await MockSFS.deploy();

    const SfsNftInitialize = await ethers.getContractFactory("SfsNftInitialize");
    sfsNftInitializeContract = await SfsNftInitialize.deploy(sfsContract.address, feeReceiver.address);
    ...
    metadataContract1 = await FlexiPunkMetadata.deploy(sfsContract.address, sfsNftTokenId);
    metadataContract2 = await FlexiPunkMetadata.deploy(sfsContract.address, sfsNftTokenId);
